...
// Pure path predicates factored out of src/path-guard.ts.
 ...
 */
export function realpathWalk(resolved: string): string {
...
export function resolveReal(...segments: string[]): string {
  return realpathWalk(path.resolve(...segments));
}

export const HOOKS_DIR = path.resolve(HOME, ".claude", "hooks");
export const HOOKS_DIR_REAL = resolveReal(HOME, ".claude", "hooks");

// Sensitive directories and files: [resolved_path, display_name, policy]
export const SENSITIVE_DIRS: SensitivePathEntry[] = [
...
export const HOOK_BLOCK_TOOLS: Set<string> = new Set(["Write", "Edit", "MultiEdit", "NotebookEdit"] satisfies WriteTool[]);
export const HOOK_READONLY_TOOLS: Set<string> = new Set(["Read", "Glob", "Grep"] satisfies ReadTool[]);

export const SENSITIVE_BASENAMES: SensitivePathEntry[] = [
