...
#
# Build artifacts are committed locally before release; CI only creates
# the GitHub release with auto-generated changelog.
# =============================================================================
...
# Pull latest from origin so we don't conflict
echo ""
...
tmp=$(mktemp)
jq --arg v "$new_version" '.version = $v' package.json > "$tmp" && mv -f "$tmp" package.json
tmp=$(mktemp)
jq --arg v "$new_version" '.version = $v' .claude-plugin/plugin.json > "$tmp" && mv -f "$tmp" .claude-plugin/plugin.json
